...
namespace HyFive.Services.Facility
{
    public class GetFacilitiesWithSessions
    {
        ...
        public class Handler : IRequestHandler<Query, List<FacilityOverviewReport>>
        {
            ...
            public async Task<List<FacilityOverviewReport>> Handle(Query query, CancellationToken cancellationToken)
            {
                ...
                var facilities = await _context.Facility
                    ...
                    .Where(x => query.FacilityId == null || query.FacilityId == x.Id)
                    .OrderBy(i => i.Name)
                    .ToListAsync(cancellationToken);

                var reports  = _mapper.Map<List<Domain.Place.Facility>, List<FacilityOverviewReport>>(facilities);
