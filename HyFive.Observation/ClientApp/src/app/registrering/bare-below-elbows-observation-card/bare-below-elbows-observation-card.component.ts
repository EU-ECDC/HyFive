...
@Component({
  selector: 'app-bare-below-elbows-observation-card',
  templateUrl: './bare-below-elbows-observation-card.component.html',
  animations: [
    Animations.swipeLeftRight
  ]
})
export class BareBelowElbowsObservationCardComponent extends BaseCardSwipe implements OnInit {
  ...
  canNotSave(): boolean {
    ...
    if (numberOfHandJewelrySelected  < 1) {
      this.observationMissingText = this.translate.instant("Bare Below Elbows missing");
  ...
  registerObservation() {
    let observation = {
      ...
      role: this.card.role,
      handJewelries: this.handJewelrySelection.reduce((acc, item) => {
